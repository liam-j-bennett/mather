...
func TestDivideCommand(t *testing.T) {
	tests := []struct {
		...
		{
			"Valid number division float",
			[]string{"3", "2"},
			[]int{3, 2},
			1.5,
			"Numbers divide to 1.5\n",
			nil,
		},
		{
			"Valid number division 5sf float",
			[]string{"4", "3"},
			[]int{4, 3},
			1.3333333333,
			"Numbers divide to 1.3333\n",
			nil,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.expectError == nil {
				mockClient.EXPECT().Divide(test.expectArgs[0], test.expectArgs[1]).Return(test.expectReturn, nil)
			}
			output := &strings.Builder{}
			cmd := newDivideCommand(mockClient)
